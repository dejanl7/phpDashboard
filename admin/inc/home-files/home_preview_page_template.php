<div class="main">
				  ...
				  <div class="grid images_3_of_2">
					<ul id="etalage">
						<li class="col-lg-12 col-md-12 col-sm-11 col-xs-11">
							<img class="etalage_thumb_image" src="../admin/img/articles_images/<?php 
								...
								?>" />
							<img class="etalage_source_image" src="../admin/img/articles_images/<?php 
									$etalage_first_img = $base->clear_string($_GET['article_id']);
									$result_of_search = articles::find_this_id($etalage_first_img);
									echo $result_of_search->article_img;
								?>" title="" />
					...
					?>
						...
						<li>
							<img class="etalage_thumb_image" src="../admin/img/articles_images/<?php echo $result->article_img_name; ?> " />
							<img class="etalage_source_image" src="../admin/img/articles_images/<?php echo $result->article_img_name; ?> "   title="" />
						</li>
				...
				<div class="product-details">
					...
					<div class="col-xs-11 rating-xs" id="rating_container" >	
						<div id="rating_for_showing">
							<?php
								$show_hide_content_marks = "SELECT * FROM preview_page WHERE user_id = ". $base->clear_string($_GET['user_id']);
	...
	<div class="product_desc">	
		<div id="horizontalTab">
			<ul class="resp-tabs-list" id="show_hide_marks_possibility_container">
			  <div id="show_hide_marks_possibility">	
			   ...
			   <?php 
					$show_hide_specification_marks = "SELECT * FROM preview_page WHERE user_id = '{$base->clear_string($_GET['user_id'])}'";
			...
			<div class="resp-tabs-container">
			  <div id="edit_article_container" rel="<?php echo $base->clear_string($_GET['article_id']); ?>">
				<div id="edit_article_info" class="product-specifications" style="
					font-size: <?php echo preview_page::css('preview_page',"preview_specifications_font_weight", $base->clear_string($_GET['user_id'])); ?>;	
					font-family: <?php echo preview_page::css('preview_page',"preview_specifications_font_type", $base->clear_string($_GET['user_id'])); ?>;
			...
			<div class="review">
				...
				<div class="your-review"><br/>
					<form method="POST" id="preview_marks_comment_form1" action="../admin/inc/pages/pages_insert_info.php">
						<div class="hidden" >
					    	<p><label>User Id<span class="red">*</span></label></p>
					    	<input type="text" class="form-control" name="user_id" value="<?php echo $base->clear_string($_GET['user_id']); ?>" required>
					    </div>
		...
		<?php 
			$query = preview_page::show_hide_comment($base->clear_string($_GET['user_id']));
			$comments_info = articles_marks::check_comment_exists($base->clear_string($_GET['article_id']));

			$current_page = !empty($_GET['page']) ? (int)$_GET['page'] : 1;
			$number_per_page = 5;
			$total_items = articles_marks::count_article_records($base->clear_string($_GET['article_id']), $base->clear_string($_GET['user_id']));
			$pagination = new pagination($current_page, $number_per_page, $total_items);


			$comments_query = "SELECT * FROM articles_marks WHERE user_id=".$base->clear_string($_GET['user_id'])." AND article_id=". $base->clear_string($_GET['article_id']). " AND client_comment!='' AND client_approve_comment='1'  ORDER BY article_mark_id DESC LIMIT {$number_per_page} OFFSET {$pagination->offset()}";
			  ...
			  <div class="comments-container">
				<div class="col-sm-12 col-xs-12 comments">
					<div id="comments_container">
						<div class="text-center" id="comments_headline" style="
							font-size: <?php echo preview_page::css('preview_page',"preview_commentsHeadline_font_weight", $base->clear_string($_GET['user_id'])); ?>;	
							font-family: <?php echo preview_page::css('preview_page',"preview_commentsHeadline_font_type", $base->clear_string($_GET['user_id'])); ?>;
						">
							Komentari
						</div>
								...
								<div class="col-sm-12 col-xs-12 comments-div" style="visibility: <?php echo $comment_visibility; ?>;">
									...
									<div id="comment-content-container">
										<div id="comment-content" class="comment-content" style="
											font-size: <?php echo preview_page::css('preview_page',"preview_commentsContent_font_weight", $base->clear_string($_GET['user_id'])); ?>;	
											font-family: <?php echo preview_page::css('preview_page',"preview_commentsContent_font_type", $base->clear_string($_GET['user_id'])); ?>;">
				...
				<div class="paginate text-center">
					<div class="comments-paginate-container" >
						<ul class='pagination text-center' data-articleid = "<?php echo $base->clear_string($_GET['article_id']); ?>" data-userid = "<?php echo $base->clear_string($_GET['user_id']); ?>">
						<?php 
							if($pagination->sum_of_pages() > 1){
								if($pagination->is_previous_page()){
									echo "<li><a href='company-preview.php?user_id=". $base->clear_string($_GET['user_id']) ."&article_id=". $base->clear_string($_GET['article_id'])."&page={$pagination->previous_page()}' rel= '{$pagination->previous_page()}'>&laquo</a></li>";
								}
								
									for($i=1; $i<=$pagination->sum_of_pages(); $i++){
										if($i == $pagination->current_page){
											echo "<li><a class='active btn btn-success' href='company-preview.php?user_id=". $base->clear_string($_GET['user_id']) ."&article_id=". $base->clear_string($_GET['article_id']). "&page='{$i}'  rel='{$i}'>$i</a></li>"; // Show current page in pagination slider !!!
										}
											else{
												echo "<li><a href='company-preview.php?user_id=". $base->clear_string($_GET['user_id']) ."&article_id=". $base->clear_string($_GET['article_id'])."&page={$i}' rel='{$i}'>$i</a></li>";
											}
									}
								
								if($pagination->is_next_page()){
									echo "<li><a href='company-preview.php?user_id=". $base->clear_string($_GET['user_id']) ."&article_id=". $base->clear_string($_GET['article_id'])."&page={$pagination->next_page()}' rel='{$pagination->next_page()}'> &raquo</a></li>";
								}
	...
	<div class="row rightsidebar span_3_of_1 sidebar" id="rightsidebar">
		<div id="article-sidebar"><a href="company-gallery.php?user_id=<?php echo $base->clear_string($_GET['user_id']); ?>">Novo iz asortimana</a></div>
		<ul class="row popular-products">
		<?php 
			$search_new_articles = "SELECT * FROM articles WHERE user_id = '{$base->clear_string($_GET['user_id'])}' ORDER BY article_uploaded_time DESC LIMIT 3";
		...
		?>
			<li class="border-info">
				<h2>	
					<a class="label" href="company-preview.php?user_id=<?php echo $base->clear_string($_GET['user_id']); ?>&article_id=<?php echo $new_articles->article_id; ?>">
						<?php echo $new_articles->article_name; ?>
					</a>
				</h2>
				<a href="company-preview.php?user_id=<?php echo $base->clear_string($_GET['user_id']); ?>&article_id=<?php echo $new_articles->article_id; ?>">
					<img src="../admin/img/articles_images/<?php echo $new_articles->article_img; ?>" alt="" />
				</a>
				  	<div class="price-details">
				        <div class="price-number">
							<p><span class="rupees"><?php echo number_format("{$new_articles->article_price}",2,",","."); ?> <?php echo $new_articles->valute; ?> </span></p>
					       		...
					       		<div class="add-cart">								
									<h4><a href="company-preview.php?user_id=<?php echo $base->clear_string($_GET['user_id']); ?>&article_id=<?php echo $new_articles->article_id; ?>">Informacije</a></h4>
							     </div>
						<div class="clear"></div>
					</div>					 
			</li>
